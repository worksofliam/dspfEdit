...
namespace dspfEdit
{
    class DragLabel : Label
    {
        ...
        void labelWorker_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            labelWorker.Location = Editor.DSPFtoUILoc(fieldInfo.Position);
        ...
        private static Point PointToDSPF(Point Location, int fieldLen)
        {
            ...
            if (x < 1) x = 1;
            if ((x + fieldLen) > Editor.WindowSize.Width) x = Editor.WindowSize.Width - fieldLen;
            if (y < 1) y = 1;
            if (y > Editor.WindowSize.Height) y = Editor.WindowSize.Height;
